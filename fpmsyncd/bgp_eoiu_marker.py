...
class BgpStateCheck():
    ...
    def set_bgp_eoiu_marker(self, family):
        db = swsssdk.SonicV2Connector(host='127.0.0.1')
        db.connect(db.STATE_DB, False)
        key = "BGP_STATE_TABLE|%s|eoiu" % family
        db.set(db.STATE_DB, key, 'reached', 'true')
...
def main():
    ...
    if bgp_state_check.bgp_ipv4_eoiu:
        bgp_state_check.set_bgp_eoiu_marker("ipv4")
    if bgp_state_check.bgp_ipv6_eoiu:
        bgp_state_check.set_bgp_eoiu_marker("ipv6")
